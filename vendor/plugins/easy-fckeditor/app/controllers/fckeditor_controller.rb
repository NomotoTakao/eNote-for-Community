...
class FckeditorController < ActionController::Base
  ...
  def get_folders_and_files(include_files = true)
    ...
    @files = {}
    begin
      @fck_url = upload_directory_path.gsub("//", "/")
  ...
  def create_folder
    begin
      ...
      path = @fck_url + params[:NewFolderName] + "/" + params[:function_name] + "/" + params[:sub_directory]
      if !(File.stat(@fck_url).writable?)
        ...
        path = @fck_url + params[:NewFolderName] + "/" + params[:function_name]
        Dir.mkdir(path,0775)
        # 一段階ずつしか作れない。
        path = @fck_url + params[:NewFolderName] + "/" + params[:function_name] + "/" + params[:sub_directory]
        Dir.mkdir(path,0775)
  ...
  def current_directory_path
    base_dir = "#{UPLOADED_ROOT}/#{params[:Type]}/#{params[:function_name]}"
    Dir.mkdir(base_dir,0775) unless File.exists?(base_dir)
    # 一段階ずつしか作成できない。
    base_dir = "#{UPLOADED_ROOT}/#{params[:Type]}/#{params[:function_name]}/#{params[:sub_directory]}"
    Dir.mkdir(base_dir,0775) unless File.exists?(base_dir)
    #check_path("#{base_dir}#{params[:CurrentFolder]}/#{params[:function_name]}")
  ...
  def upload_directory_path
    ...
    #  パラメータに含まれる"function_name"の値のサブディレクトリを表示する。
    # 一つの文字列にすると、params[CurrentFolder]がnilのとき文字列の結合に不具合が起きるので、分割している。
    "#{uploaded}#{params[:CurrentFolder]}" + "/" + "#{params[:function_name]}/#{params[:sub_directory]}"
  end
