...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    japanese_month = ["1月", "2月", "3月", "4月",  "5月",  "6月",
                      "7月", "8月", "9月", "9月", "10月", "11月", "12月"]
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      ...
      cell_attrs[:class] += " holiday" if holidays.include?(d)
      cell_attrs[:class] += " event" if events.include?(d)
    ...
    first.upto(last) do |cur|
      ...
      cell_text  ||= cur.mday
      cell_attrs ||= {:class => options[:day_class]}
      cell_attrs[:class] += " holiday" if holidays.include?(cur)
      cell_attrs[:class] += " event" if events.include?(cur)
