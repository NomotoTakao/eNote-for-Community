  ...
  module Ja
    ...
    def quote_if_necessary_with_ja(text, charset)
      ...
      if auto_base64_encode
        #〜の字が化けてしまうのでこう書く　--2009/11/25 ライトハウス片山
#        text = text.gsub("\xef\xbd\x9e", "\xe3\x80\x9c")
#        NKF.nkf('-jW -m0 --oc=CP50220 --cp932', text).strip
#        text = text.gsub("\xef\xbd\x9e", "\xe3\x80\x9c")
#        text = NKF.nkf('-j -m0 --oc=CP50220 --cp932inv', text)
#        text = NKF.nkf('-j -m0', text)
#        text = [text].pack('m').delete("\r\n")
#        "=?#{charset}?B?#{text}?="
        NKF.nkf('-jW -M --oc=CP50220',text)
      else
    ...
    def parse_mail_addr(recipient)
      #どうもこの記述があると動作がおかしいのでコメント　--2009/11/25 ライトハウス片山
#      /(.*?)<(.*?)>/ =~ recipient
#      return $& ? $2 : recipient
      recipient
    end
