...
//
//
//@param  argCheckValue		（I）チェックする文字列
//
...
//		  異常：false
//-------------------------------------------------------------------------------------------------
function inValueChk(argCheckValue,argFlg,WildCardFlg,
  ...
  //必須入力チェック
  if (wkinStr == null || wkinStr.length == 0) {
    if	(wkmustflg	==	1)	{
  ...
  for (i = 0;  i < wkinStr.length;  i++){
    ...
    if (isStr(chk_wkflg,WildCardFlg,Wk_Unicode,wkinStr)) {
  ...
  if (!chk_flg){
    ...
    //7:使用しない
    case 7:
      window.alert(wkmsg + "には日付書式（YYYY-MM-DD または YYYYMMDD）の入力が可能です。");
      break swlabel;
    ...
    case 13:
      ...
      break swlabel;
    case 14:
      window.alert(wkmsg + "は不正なメールアドレスです。");
      break swlabel;
    case 15:
        window.alert(wkmsg + "には半角数字、「 - 」の入力が可能です。");
        break swlabel;
...
/****************************************************************
* 機　能： 入力された値が日付でYYYY-MM-DD or YYYYMMDD形式になっているか調べる
*        その後YYMMDD形式の場合は、YYYY-MM-DDに変換して画面に表示する
* 引　数： datestr　入力された値
*        culum_name　チェック対象のカラム名
*        disp_str　チェック対象の項目名
* 戻り値： 正：true　不正：false
****************************************************************/
function checkDateFormat(date_str, culum_name, disp_str){
  //日付の形式チェック
  if (!inValueChk(date_str,7,0,0,0,0,disp_str)) {
    return false;
  }
  //ハイフン区切りにする
  if (date_str.length == 8) {
    year = date_str.substring(0, 4);
    month = date_str.substring(4, 6);
    day = date_str.substring(6, 8);
    $(culum_name).val(year + "-" + month + "-" + day);
  }
  return true;
}
...
/****************************************************************
* 機　能： 日付の大小チェックを行う
* 引　数： compare_flg　true:日付(YYYYMMDD)の比較
*                    false:日時(YYYYMMDDhhmm)の比較
*        date_from_str, hour_from_str, min_from_str　開始日時
*        date_to_str, hour_to_str, min_to_str　終了日時
*        disp_str　チェック対象の項目名
* 戻り値： 正：true　不正：false
****************************************************************/
function compareDate(compare_flg, date_from_str, hour_from_str, min_from_str, date_to_str, hour_to_str, min_to_str, disp_str){
  //日付の比較
  if (compare_flg == true) {
    var time_from = Date.parse(date_from_str.substring(0, 4) + "/" + date_from_str.substring(5, 7) + "/" + date_from_str.substring(8));
    var time_to = Date.parse(date_to_str.substring(0, 4) + "/" + date_to_str.substring(5, 7) + "/" + date_to_str.substring(8));
  //日時の比較
  } else {
    var time_from = Date.parse(date_from_str.substring(0, 4) + "/" + date_from_str.substring(5, 7) + "/" + date_from_str.substring(8)
        + " " + hour_from_str + ":" + min_from_str + ":00");
    var time_to = Date.parse(date_to_str.substring(0, 4) + "/" + date_to_str.substring(5, 7) + "/" + date_to_str.substring(8)
        + " " + hour_to_str + ":" + min_to_str + ":00");
  }
  if (time_from > time_to) {
      alert(disp_str + "の大小関係を正しく指定してください。");
      return false;
  }
  return true;
}
