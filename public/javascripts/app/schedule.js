/*
* タイマーストップ
*/
function timerStop(){
  clearInterval(timer);
}

/*
 * タイマースタート
 */
function timerStart(){
  timer=setInterval('timers()',300000);
}

/*
 * ダイアログの初期化
 */
//空き状況確認用(同時登録)
function dialog_spare_time_other_member(){
  jQuery("#dialog_spare_time_other_member").dialog({
      autoOpen:  false,
      modal:  false,
      width: 1000,
      height: 650,
      buttons: {
      "閉じる": function() {
          jQuery(this).dialog("close");
      }
  }
  });
}

//空き状況確認用(施設)
function dialog_spare_time_facility(){
  jQuery("#dialog_spare_time_facility").dialog({
      autoOpen:  false,
      modal:  false,
      width: 1000,
      height: 650,
      buttons: {
        "閉じる": function() {
          jQuery(this).dialog("close");
        },
        "選択": function() {
          //今回選択された施設を取得
          var facility_cd_colon = "";
          $(".check_facility_cd:input:checked").each(function(){
            if(facility_cd_colon == ""){
              facility_cd_colon += $(this).attr("id").replace("facilitis_", "");
            } else {
              facility_cd_colon += "," + $(this).attr("id").replace("facilitis_", "");
            }
          });
          //既に選択されている施設を取得
          select_facility_list_old = document.getElementById("decided_facility_new").value

          //画面遷移
          jQuery("#facility_list").load("/schedule/reserve/facility_list?return_mode=1&facility_cd_colon=" + facility_cd_colon
              + "&facility_all_list_colon=" + $("#facility_all_list_colon").val()
              + "&select_facility_list_old=" + select_facility_list_old);
          jQuery(this).dialog("close");
        }
      }
  });
}

/*
* ダイアログ表示
*/
//追加用
function dialog_reserve_open_ins(date, user_cd){
  //タイマーストップ
  timerStop();
  //一覧でメンバーのリンクをクリックされた場合
  if (user_cd != "") {
    select_proxy_user_cd = user_cd;
    back_secretaries_user_cd = "";
  //遷移元のリストボックス選択情報(月,リスト)
  } else if ($("#secretaries_user_cd_h").val() != undefined) {
    select_proxy_user_cd = $("#secretaries_user_cd_h").val();
    back_secretaries_user_cd = $("#secretaries_user_cd_h").val();
  } else {
    select_proxy_user_cd = "";
    back_secretaries_user_cd = "";
  }
  //遷移元のリストボックス選択情報(週,日)
  if ($("#belong_list_h").val() != undefined) {
    back_belong_index = $("#belong_list_h").val();
  } else {
    back_belong_index = "";
  }

  //画面遷移
  document.location = "/schedule/reserve/new?date=" + encodeURIComponent(date)
        + "&select_proxy_user_cd=" + select_proxy_user_cd
        + "&back_secretaries_user_cd=" + back_secretaries_user_cd
        + "&back_belong_index=" + back_belong_index;
  return false;
}

//更新用
function dialog_reserve_open_edit(id, repeat_flg, user_cd){
  //タイマーストップ
  timerStop();
  //遷移元のリストボックス選択情報(月,リスト)
  if ($("#secretaries_user_cd_h").val() != undefined) {
    back_secretaries_user_cd = $("#secretaries_user_cd_h").val();
  } else {
    back_secretaries_user_cd = "";
  }
  //遷移元のリストボックス選択情報(週,日)
  if ($("#belong_list_h").val() != undefined) {
    back_belong_index = $("#belong_list_h").val();
  } else {
    back_belong_index = "";
  }

  //画面遷移
  document.location = "/schedule/reserve/" + id + "/edit?repeat_flg=" + repeat_flg
      + "&back_secretaries_user_cd=" + back_secretaries_user_cd
      + "&back_belong_index=" + back_belong_index
      + "&select_proxy_user_cd=" + user_cd;
  return false;
}

//空き状況確認用(同時登録)
function dialog_spare_time_other_member_open(){
  /*開始日の必須チェック*/
  if (!inValueChk($("#d_schedule_plan_date_from").val(),99,0,1,0,0,"開始日")) {
    return false;
  }
  /*開始日の妥当性チェック*/
  result = checkDateFormat($("#d_schedule_plan_date_from").val(), "#d_schedule_plan_date_from", "開始日");
  if (!result) {
    return result;
  }

  //データ取得
  //日付
  date = $("#d_schedule_plan_date_from").val();
  //メインユーザー
  select_proxy_user_cd = $("#select_proxy_user_cd").val();
  //選択されたメンバー
  member = document.getElementById("decided_member_new").value;

  //画面遷移
  jQuery("#dialog_spare_time_other_member").load("/schedule/reserve/spare_time_other_member?day=" + date
      + "&select_proxy_user_cd=" + select_proxy_user_cd + "&member=" + member);
  jQuery("#dialog_spare_time_other_member").dialog("open");
  return false;
}

//空き状況確認用(施設)
function dialog_spare_time_facility_open(){
  /*開始日の必須チェック*/
  if (!inValueChk($("#d_schedule_plan_date_from").val(),99,0,1,0,0,"開始日")) {
    return false;
  }
  /*開始日の妥当性チェック*/
  result = checkDateFormat($("#d_schedule_plan_date_from").val(), "#d_schedule_plan_date_from", "開始日");
  if (!result) {
    return result;
  }

  //データ取得
  //日付
  date = $("#d_schedule_plan_date_from").val();
  //メインユーザー
  select_proxy_user_cd = $("#select_proxy_user_cd").val();
  //選択された施設グループ
  facility_group_cd = $("#facility_undecide").val();
  //選択された施設
  select_facility_list = document.getElementById("decided_facility_new").value

  //画面遷移
  jQuery("#dialog_spare_time_facility").load("/schedule/reserve/spare_time_facility?day=" + date
      + "&select_proxy_user_cd=" + select_proxy_user_cd + "&facility_group_cd=" + facility_group_cd
      + "&select_facility_list=" + select_facility_list);
  jQuery("#dialog_spare_time_facility").dialog("open");
  return false;
}

function displayError(){
  alert("ファイルの取得に失敗しました");
}

/*
 * 同時登録の矢印クリック時の処理(選択データのみ)
 */
function ClickMoveMember(moveMode) {
  var undBox = document.getElementById("undecided_member_");
  var dBox = document.getElementById("decided_member_");
  var leftBox = undBox;
  var rightBox = dBox;
  var fromBox, toBox;

  //移動元と移動先を決定する
  if (moveMode == 0) {
    fromBox = leftBox;
    toBox = rightBox;
  } else if (moveMode == 1) {
    fromBox = rightBox;
    toBox = leftBox;
  }

  //メンバーの移動を行う
  if ((fromBox != null) && (toBox != null)) {
    if(fromBox.length < 1 || fromBox.selectedIndex == -1) {
      alert("移動するメンバーを選択してください。");
      return false;
    }
    while (fromBox.selectedIndex >= 0) {
      var newOption = new Option();
      //選択値のチェック
      if (moveMode == 0) {
        for (var i = 0; i <dBox.length; i++) {
          if (fromBox.options[fromBox.selectedIndex].value == dBox.options[i].value) {
            alert("既に選択されているメンバーです。");
            return false;
          }
        }
      }
      newOption.text = fromBox.options[fromBox.selectedIndex].text;
      newOption.value = fromBox.options[fromBox.selectedIndex].value;
      toBox.options[toBox.length] = newOption;
      fromBox.remove(fromBox.selectedIndex);
    }
  }
  //選択決定エリアのデータを全て取得
  var member = new Array(dBox.length);
  for (var i = 0; i < dBox.options.length; i++) {
    member[i] = dBox.options[i].value;
  }
  document.getElementById("decided_member_new").value = member;
}

 /*
  * 同時登録の矢印クリック時の処理(全て)
  */
 function ClickMoveAllMember(moveMode) {
   var undBox = document.getElementById("undecided_member_");
   var dBox = document.getElementById("decided_member_");
   var leftBox = undBox;
   var rightBox = dBox;
   var fromBox, toBox;

   //移動元と移動先を決定する
   if (moveMode == 0) {
     fromBox = leftBox;
     toBox = rightBox;
   } else if (moveMode == 1) {
     fromBox = rightBox;
     toBox = leftBox;
   }
   //メンバーの移動を行う
   if ((fromBox != null) && (toBox != null)) {
     if(fromBox.length < 1) {
       alert("移動するメンバーを選択してください。");
       return false;
     }
   }
   //全てを選択状態にする
   for (var i = 0; i < fromBox.options.length; i++) {
     fromBox.options[i].selected = true;
   }
   //同時登録の矢印クリック時の処理(選択データのみ)
   ClickMoveMember(moveMode);
 }

/*
 * 入力チェック
 */
function CheckValidate(){
  /*タイトルの必須チェック*/
  if (!inValueChk($("#reserve_title").val(),99,0,1,0,0,"タイトル")) {
    return false;
  }
  /*開始日の必須チェック*/
  if (!inValueChk($("#d_schedule_plan_date_from").val(),99,0,1,0,0,"開始日")) {
      return false;
  }
  /*終了日の必須チェック*/
  if (!inValueChk($("#d_schedule_plan_date_to").val(),99,0,1,0,0,"終了日")) {
      return false;
  }
  /*開始日の妥当性チェック*/
  result = checkDateFormat($("#d_schedule_plan_date_from").val(), "#d_schedule_plan_date_from", "開始日");
  if (!result) {
    return result;
  }
  /*終了日の妥当性チェック*/
  result = checkDateFormat($("#d_schedule_plan_date_to").val(), "#d_schedule_plan_date_to", "終了日");
  if (!result) {
    return result;
  }
  /*開始日時と終了日時の大小チェック*/
  result = compareDate(
      $("#reserve_plan_allday_flg").attr('checked'),
      $("#d_schedule_plan_date_from").val(), $("#reserve_plan_time_from_4i").val(), $("#reserve_plan_time_from_5i").val(),
      $("#d_schedule_plan_date_to").val(), $("#reserve_plan_time_to_4i").val(), $("#reserve_plan_time_to_5i").val(),
      "開始日付と終了日付");
  if (!result) {
    return result;
  }

  /*通知日程の必須チェック*/
  if ($("#reserve_reminder_day_flg").attr('checked') == true
    || $("#reserve_reminder_week_flg").attr('checked') == true
    || $("#reserve_reminder_month_flg").attr('checked') == true
    || $("#reserve_reminder_specify_flg").attr('checked') == true) {
    /*通知方法の必須チェック*/
    if ($("#reserve_reminder_mypage_flg").attr('checked') == false
      && $("#reserve_reminder_email_flg").attr('checked') == false
      && $("#reserve_reminder_mobile_mail_flg").attr('checked') == false) {
      alert("通知日程を指定した場合は、通知方法を指定してください。");
      return false;
    }
  }

  /*通知方法の必須チェック*/
  if ($("#reserve_reminder_mypage_flg").attr('checked') == true
    || $("#reserve_reminder_email_flg").attr('checked') == true
    || $("#reserve_reminder_mobile_mail_flg").attr('checked') == true) {
    /*通知日程の必須チェック*/
    if ($("#reserve_reminder_day_flg").attr('checked') == false
      && $("#reserve_reminder_week_flg").attr('checked') == false
      && $("#reserve_reminder_month_flg").attr('checked') == false
      && $("#reserve_reminder_specify_flg").attr('checked') == false) {
      alert("通知方法を指定した場合は、通知日程を指定してください。");
      return false;
    }
  }

  //指定日がチェックされている場合
  if ($("#reserve_reminder_specify_flg").attr('checked') == true) {
    /*指定日の必須チェック*/
    if (!inValueChk($("#d_schedule_reminder_specify_date").val(),99,0,1,0,0,"指定日")) {
      return false;
    }
    /*指定日の妥当性チェック*/
    result = checkDateFormat($("#d_schedule_reminder_specify_date").val(), "#d_schedule_reminder_specify_date", "指定日");
    if (!result) {
      return result;
    }
  }

  //繰り返し登録する場合
  if ($("#reserve_repeat_flg").attr('checked') == true) {
    /*繰り返し終了日の必須チェック*/
    if (!inValueChk($("#d_schedule_repeat_date_to").val(),99,0,1,0,0,"繰り返し終了日")) {
      return false;
    }
    /*繰り返し終了日の妥当性チェック*/
    result = checkDateFormat($("#d_schedule_repeat_date_to").val(), "#d_schedule_repeat_date_to", "繰り返し終了日");
    if (!result) {
      return result;
    }
    /*繰り返し終了日の指定可能日チェック*/
    //会社Mのデータを基に期のリストを作成する(0:1期末日, 1:2期末日..x:1期末日の1年後)
    term_list_work = $("#term_colon").val().split(",");
    term_list = new Array(term_list_work.length + 1);
    date_s = $("#d_schedule_plan_date_from").val();  //開始日(STRING型:YY-MM-DD)
    date_d = new Date(date_s.substring(0,4), date_s.substring(5,7)-1, date_s.substring(8,10));  //開始日(DATE型)
    //リスト作成
    for (i=0; i<term_list_work.length; i++) {
      term_list[i] = GetTermDate(date_s, term_list_work[i], 0);
    }
    term_list[term_list_work.length] = GetTermDate(date_s, term_list_work[0], 1); //1年後
    //直近の期末日を判定
    term_old = term_list[0];
    term_date = term_old;
    for (i=1; i<term_list.length; i++) {
      term_new = term_list[i];
      if ((term_old <= date_d) && (date_d < term_new)) {
        term_date = term_new;
        break;
      } else {
        term_old = term_new;
      }
    }
    //チェック
    repeat_date_to_s = $("#d_schedule_repeat_date_to").val();	//繰り返し終了日(STRING型:YY-MM-DD)
    repeat_date_to_d = new Date(repeat_date_to_s.substring(0,4), repeat_date_to_s.substring(5,7)-1, repeat_date_to_s.substring(8,10));  //繰り返し終了日(DATE型)
    term_date_disp = term_date.getFullYear() + "年" + (term_date.getMonth() + 1) + "月" + term_date.getDate() + "日";
    if (repeat_date_to_d > term_date) {
        alert("繰り返し終了日は「" + term_date_disp + "」以前の日付を指定してください。");
        return false;
    }
    /*繰り返し間隔の必須チェック*/
    if ($('input:radio[name=reserve[repeat_interval_flg]]:checked').val() == null) {
      alert("繰り返し間隔を指定してください。");
      return false;
    }
    //曜日指定の場合
    /*曜日の必須チェック*/
    if ($('input:radio[name=reserve[repeat_interval_flg]]:checked').val() == 2) {
      if ($("#reserve_repeat_week_sun_flg").attr('checked') == false
          && $("#reserve_repeat_week_mon_flg").attr('checked') == false
          && $("#reserve_repeat_week_tue_flg").attr('checked') == false
          && $("#reserve_repeat_week_wed_flg").attr('checked') == false
          && $("#reserve_repeat_week_thu_flg").attr('checked') == false
          && $("#reserve_repeat_week_fri_flg").attr('checked') == false
          && $("#reserve_repeat_week_sat_flg").attr('checked') == false) {
        alert("曜日を指定してください。");
        return false;
      }
    }
  }

  return true;
}

/*
 * 更新データチェック
 */
function CheckUpdateData(){
  if (document.getElementById("decided_member_old").value != ""
    && document.getElementById("decided_member_new").value == "") {
      result = confirm("同時登録メンバーが削除されましたが\n宜しいですか？");
      if (!result) {
      return false;
      }
  }
  return true;
}

/*
 * 期末の日付を設定
 */
function GetTermDate(select_date, term_end, over_flg) {
  //年を跨ぐ場合
  if (over_flg == 1) {
    date = new Date(select_date.substring(0,4), select_date.substring(5,7)-1+12, select_date.substring(8,10));
  //年を跨がない場合
  } else {
    date = new Date(select_date.substring(0,4), select_date.substring(5,7)-1, select_date.substring(8,10));
  }
  year = date.getFullYear();
  term_end_work = year + term_end;
  term_end_yymmdd = new Date(term_end_work.substring(0,4), term_end_work.substring(4,6)-1, term_end_work.substring(6,8));
  return term_end_yymmdd;
}

/*
 * 終日のチェックボックスクリック時
 */
function ClickAllDayCheck(){
  //終日がチェックされた場合
  if ($("#reserve_plan_allday_flg").attr('checked') == true) {
    con_flg = true;
  //終日のチェックが外れた場合
  } else {
    con_flg = false;
  }

  //時間プルダウンを非活性にする
  document.getElementById("reserve_plan_time_from_4i").disabled = con_flg;  //開始時間
  document.getElementById("reserve_plan_time_from_5i").disabled = con_flg;
  document.getElementById("reserve_plan_time_to_4i").disabled = con_flg;    //終了時間
  document.getElementById("reserve_plan_time_to_5i").disabled = con_flg;
}

/*
 * 処理中ダイアログ表示
 */
function LoadBlockUi() {
  $.blockUI({css:{ backgroundColor:'#0099FF', color:'#fff' },baseZ: 2000,message: '<h1>お待ちください...</h1>'});
}
